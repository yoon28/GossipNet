...
def get_sample_weights(num_classes, labels):
    ...
    with tf.variable_scope('loss_weighting'):
        exp_class_weights = tf.constant(
            [1.0 - pos_weight] + [pos_weight / (num_classes - 1)] *
            (num_classes - 1),
            dtype=tf.float32)
...
def weighted_logistic_loss(logits, labels, instance_weights):
    ...
    return tf.nn.sigmoid_cross_entropy_with_logits(logits * instance_weights,
                                                   labels)
...
def get_resnet(image_tensor, reuse):
    with slim.arg_scope(resnet_v1.resnet_arg_scope(is_training=False)):
        ...
        net, end_points = net_fun(
            image, global_pool=False, output_stride=16, reuse=reuse)
...
def crop_windows(imfeats, boxdata, stride):
    ...
    # n_boxes = tf.stack([tf.shape(boxdata[0])[0]])
    ...
    detection_feats, _ = roi_pooling_op.roi_pool(
        imfeats,
        frcn_boxes,
        pooled_height=cfg.imfeat_crop_height,
        pooled_width=cfg.imfeat_crop_width,
...
class Gnet(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope('gnet', reuse=reuse):
            with tf.variable_scope('preprocessing'):
                ...
                pw_feats = (
                    self._geometry_feats(pair_c_idxs, pair_n_idxs) *
    ...
    def _pw_feats_fc(pw_feats, weights_init, biases_init, weight_reg):
        ...
        for i in range(1, cfg.gnet.num_pwfeat_fc):
            feats = tf.contrib.layers.fully_connected(
                inputs=feats,
                num_outputs=cfg.gnet.pwfeat_dim,
                activation_fn=tf.nn.relu,
                weights_initializer=weights_init,
                weights_regularizer=weight_reg,
                biases_initializer=biases_init,
        ...
        feats = tf.contrib.layers.fully_connected(
            inputs=feats,
            num_outputs=cfg.gnet.pwfeat_narrow_dim,
            activation_fn=tf.nn.relu,
            weights_initializer=weights_init,
            weights_regularizer=weight_reg,
            biases_initializer=biases_init,
    ...
    def _geometry_feats(self, c_idxs, n_idxs):
        with tf.variable_scope('pairwise_features'):
            if self.multiclass:
                ...
                det_scores = tf.scatter_nd(mc_score_idxs, self.det_scores,
                                           mc_score_shape)
            else:
            ...
            all = tf.concat([
                c_score, n_score, ious, x_dist, y_dist, l2_dist, w_diff, h_diff,
                aspect_diff
            ], 1)
